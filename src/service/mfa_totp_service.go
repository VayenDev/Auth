...
func AddMFATimedOneTimePassword(setup GeneralData[any], userUUID uuid.UUID, username string) (data.MFATimedOneTimePassword, error) {
	err := setup.Validate()
	if err != nil {
		return data.MFATimedOneTimePassword{}, err
	}
...
func ValidateTOTP(setup GeneralData[any], userUUID uuid.UUID, token string) (bool, error) {
